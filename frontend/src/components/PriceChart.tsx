...
export default function PriceChart({ detail }: { detail: MarketDetail }) {
  ...
  if (data.length === 0) {
    return <p className="muted">No snapshots recorded yet for this market.</p>;
  }

  const res = detail.resolution;
  const resolvedT = res?.resolved_at ? Date.parse(res.resolved_at) : null;
  // Market price closest to resolution — for the "how close did it get" read.
  const lastPoint = data[data.length - 1];
  const finalPrice = lastPoint.price_yes as number;
      ...
      {res && (
        <div className="panel" style={{ marginBottom: 16 }}>
          <div className="kv">
            <div>Outcome</div>
            <div>{res.outcome ?? "—"}</div>
            {"final_score" in res && res.final_score != null && (
              <>
                <div>Final score</div>
                <div>{res.final_score}</div>
              </>
            )}
            {"actual_measured_value" in res && res.actual_measured_value != null && (
              <>
                <div>Measured value</div>
                <div>{res.actual_measured_value}</div>
              </>
            )}
      ...
      <div className="chart-wrap">
        <ResponsiveContainer>
          <LineChart data={data} margin={{ top: 8, right: 16, bottom: 8, left: 0 }}>
            ...
            <Line type="monotone" dataKey="price_no" stroke="#3a4356" dot={false} strokeWidth={1} name="P(no)" isAnimationActive={false} />
            {resolvedT != null && (
              <ReferenceLine x={resolvedT} stroke="#f85149" strokeDasharray="4 3" label={{ value: "resolved", fill: "#f85149", fontSize: 11 }} />
            )}
            {resolvedT != null && (
              <ReferenceDot x={lastPoint.t} y={finalPrice} r={4} fill="#f85149" stroke="none" />
            )}
          </LineChart>
        </ResponsiveContainer>
      </div>

      <h2>Spread &amp; 24h volume</h2>
      <div className="chart-wrap small">
        <ResponsiveContainer>
          <LineChart data={data} margin={{ top: 4, right: 16, bottom: 4, left: 0 }}>
            ...
            <Line yAxisId="v" type="monotone" dataKey="volume_24h" stroke="#3fb950" dot={false} name="vol 24h" isAnimationActive={false} />
          </LineChart>
        </ResponsiveContainer>
      </div>
    </>
